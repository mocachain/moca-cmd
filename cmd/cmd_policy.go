...
Examples:
$ moca-cmd policy put --groupId 111 --actions get,delete grn:o::moca-bucket/moca-object`,
		Flags: []cli.Flag{
			&cli.Uint64Flag{
				Name:  groupIDFlag,
				Value: 0,
...
Examples:
$ moca-cmd policy rm --groupId 111  grn:o::moca-bucket/moca-object`,
		Flags: []cli.Flag{
			&cli.Uint64Flag{
				Name:  groupIDFlag,
				Value: 0,
...
Examples:
$ moca-cmd policy ls --groupId 111  grn:o::moca-bucket/moca-object`,
		Flags: []cli.Flag{
			&cli.Uint64Flag{
				Name:  groupIDFlag,
				Value: 0,
...
func putPolicy(ctx *cli.Context) error {
	...
	if expireTime > 0 {
		...
		if bucketNameOfBucketPolicy != "" && isObjectActionInBucketPolicy {
			...
			statement = utils.NewStatement(actions, effect, []string{mocadTypes.NewObjectGRN(bucketNameOfBucketPolicy, "*").String()}, sdktypes.NewStatementOptions{StatementExpireTime: &tm})
		} else {
		...
		if bucketNameOfBucketPolicy != "" && isObjectActionInBucketPolicy {
			// putting bucket policy need to set the sub-resource as "grn:o:bucket-name/*"
			statement = utils.NewStatement(actions, effect, []string{mocadTypes.NewObjectGRN(bucketNameOfBucketPolicy, "*").String()}, sdktypes.NewStatementOptions{})
		} else {
