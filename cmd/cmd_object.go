...
$ moca-cmd object get moca://moca-bucket/moca-object  file.txt `,
		Flags: []cli.Flag{
			&cli.Int64Flag{
...
Examples:
$ moca-cmd object cancel  moca://moca-bucket/moca-object`,
	}
}
...
Examples:
$ moca-cmd object ls moca://moca-bucket`,
		Flags: []cli.Flag{
			&cli.BoolFlag{
				Name:  recursiveFlag,
				Value: false,
...
Examples:
$ moca-cmd object get-progress moca://moca-bucket/moca-object`,
	}
}
...
$ moca-cmd object mirror --destChainId 97 --id 1

# Mirror a object using bucket and object name
$ moca-cmd object mirror --destChainId 97 --bucketName yourBucketName --objectName yourObjectName
`,
		Flags: []cli.Flag{
			&cli.StringFlag{
...
Examples:
$ moca-cmd object setTag --tags='[{"key":"key1","value":"value1"},{"key":"key2","value":"value2"}]' moca://moca-bucket/moca-object`,
...
func setTagForObject(ctx *cli.Context) error {
	...
	grn := mocadTypes.NewObjectGRN(bucketName, objectName)
	...
	if err = waitTxnStatus(client, c, txnHash, "SetTags"); err != nil {
		return toCmdErr(err)
	}
	...
	fmt.Printf("latest group tags on chain: %s\n", objectInfo.ObjectInfo.GetTags())
	return nil
}
...
func putObject(ctx *cli.Context) error {
	...
	if ctx.NArg() == 1 {
		...
		if err = uploadFile(bucketName, objectName, filePath, urlInfo, ctx, mocaClient, isUploadSingleFolder, true, 0); err != nil {
			return toCmdErr(err)
		}
		...
		if supportRecursive {
			...
			if err = uploadFolder(urlInfo, ctx, mocaClient); err != nil {
				return toCmdErr(err)
			}
			return nil
		}
		...
		if needUploadMultiFiles {
			...
			for idx, fileName := range filePathList {
				...
				if err = uploadFile(bucketName, objectName, fileName, urlInfo, ctx, mocaClient, false, true, objectSize); err != nil {
			...
			if err = uploadFile(bucketName, objectName, filePathList[0], urlInfo, ctx, mocaClient, false, true, objectSize); err != nil {
				return toCmdErr(err)
			}
		}
	}
...
func uploadFolder(urlInfo string, ctx *cli.Context,
	...
	fmt.Println("You can check your task status and progress by using cmd as below:\n\n- List all your tasks: ./moca-cmd task ls\n- Check status: ./moca-cmd task status --task.id taskID\n- Retry (in case this process is killed accidentally): ./moca-cmd task retry --task.id taskID\n- Delete task: ...\n\n>>================================================")
	...
	return uploadFolderByTask(ctx, homeDir, mocaClient, taskState)
}

func uploadFolderByTask(ctx *cli.Context, homeDir string, mocaClient client.IClient, taskState *TaskState) error {
	signalCtx, cancel := context.WithCancel(ctx.Context)
	go stateSync(signalCtx, homeDir, taskState)
	sealSignal := make(chan int)
	go sealChecker(ctx, taskState, mocaClient, sealSignal)
...
func sealChecker(ctx *cli.Context, taskState *TaskState, mocaClient client.IClient, signal chan int) {
	...
	for range tick.C {
		...
		headObjOutput, queryErr := mocaClient.HeadObject(ctx.Context, utj.BucketName, utj.ObjectName)
		if queryErr != nil {
			continue
		}
...
func uploadFile(bucketName, objectName, filePath, urlInfo string, ctx *cli.Context,
	mocaClient client.IClient, uploadSingleFolder, printTxnHash bool, objectSize int64,
) error {
	var file *os.File
	contentType := ctx.String(contentTypeFlag)
	secondarySPAccs := ctx.String(secondarySPFlag)
	...
	if err != nil {
		if uploadSingleFolder {
			...
			if err = waitTxnStatus(mocaClient, c, txnHash, "createObject"); err != nil {
				return toCmdErr(err)
			}
		}
		if printTxnHash {
	...
	for {
		...
		case <-ticker.C:
			count++
			headObjOutput, queryErr := mocaClient.HeadObject(c, bucketName, objectName)
			if queryErr != nil {
				return queryErr
			}
			if count%10 == 0 {
...
func uploadFileByTask(bucketName, objectName, filePath string, uploadFlag UploadFlag,
	mocaClient client.IClient, uploadSingleFolder bool, objectSize int64,
) error {
	var file *os.File
	...
	if err != nil {
		if uploadSingleFolder {
			_, err = mocaClient.CreateFolder(c, bucketName, objectName, opts)
			if err != nil {
				return toCmdErr(err)
			}
		} else {
...
func getObject(ctx *cli.Context) error {
	...
	mocaClient, err := NewClient(ctx, ClientOptions{IsQueryCmd: false, ForceToUseSpecifiedSpEndpointForDownloadOnly: spEndpoint})
	if err != nil {
		return toCmdErr(err)
	}
	...
	if resumableDownload {
		opt.PartSize = partSize
		err = mocaClient.FGetObjectResumable(c, bucketName, objectName, filePath, opt)
		if err != nil {
			return toCmdErr(err)
		}
